...
/**
Store가 도입되면 Right, Left 컴포넌트에서 Props를 통해 관리하던 값이나 
함수는 더이상 필요하지않다. 따라서 기존 코드에서 제거한다. 
 */
function Right1(){
  return (
    <div>
...
/** 
useDispatch 훅 
  : State 값을 변경할때 Reducer 함수를 호출하는 역할을 한다. 
 */
function Right3(){
  /**
  type을 PLUS로 설정하여 Store에 정의된 Reducer 를 호출한다. 
...
const Left3 = () => {
  /**
  Store에 정의된 여러개의 State중 어떤값을 받을지를 정의한 함수를 
...
function App() {
  ...
  /**
  <Provider> 컴포넌트
  : 어떤 컴포넌트에 State를 제공하지 결정하는 Wrapper 컴포넌트로 
  ...
  return (
    <div className="root">
      <h2>React - Redux </h2>
      <div id='grid'>        
        {/* 컴포넌트를 Wrapping할때 앞에서 생성한 store를 Props로 
        전달한다.  */}
