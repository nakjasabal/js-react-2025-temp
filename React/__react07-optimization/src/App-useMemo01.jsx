...
이를 위해 React에서는 useMemo, useCallback Hook을 제공한다. 
 */
...
function App() {
  ...
  /* Step1 : App컴포넌트가 렌더링되면 아래 2개의 함수를 호출하여 State값을 
    설정한다. 즉 렌더링될때마다 호출되는 구조를 가진다. */
  ...
  Step2 : 시간이 많이 걸리는 함수를 호출한 후 반환되는 값을 useMemo를
    통해 캐시에 저장한다. 이 값은 longTimeNum이 변경될때마 다시 함수를
    호출하므로 Short를 눌렀을때는 호출되지 않는다. 즉 렌더링시 불필요한
    호출을 줄일 수 있다. 
   */
  ...
  /**  
  Step1에서는 State가 변경될때마다 새롭게 렌더링 되므로 Long과 Short 함수가
  무조건 한번씩 호출된다. 따라서 Short만 변경하더라도 실행시간이 매우 오래 
  걸리게되므로 프로그램 전체의 퍼포먼스가 떨어진다. 
   */
